...
$this->db->select("lgnUserMaxProfit,lgnUserMaxLoss,lgnUserMaxStake,Commission,stakeLimit,mstrid as usecode,mstrname,mstruserid as usename,active,ipadress,usecrdt,usemodt,case when usetype=0 then 'Admin' else case when usetype=1 then 'Master' else case when usetype=2 then 'Dealer' else case when usetype=3 then 'Client' else case when usetype=4 then 'Helper' end end end end end as usetype");
		...
		function pointHistory($userId){
			$this->db->select('cm.mstrname as name,cm.mstruserid,tc.Narration,tc.Chips,tc.MstDate,tc.CrDr,DATE_FORMAT(tc.MstDate, "%d-%m-%Y") as CDate');
			$this->db->from('tblchips tc');
			$this->db->join('createmaster cm','cm.mstrid=tc.userId','INNER');
			$this->db->where('tc.userId', $userId);
			$this->db->where('tc.Chips <>', 0);
			$this->db->order_by('tc.MstDate','DESC');
			$query = $this->db->get();
			return $query->result_array();
		}
		...
		function listFancy($matchId){
			...
			$this->db->select("mf.active,mf.ID,mf.HeadName,mf.date as Date,m.matchName,m.MstCode as matchId,mf.TypeID as fType,mf.result,(case when (mf.active = 0 ) then 'Inactive'  when (mf.active = 1) then 'Active'
                      when (mf.active = 2) then 'Suspend'   end) as fstatus,mf.SprtId SportID");
			$this->db->from('matchfancy mf');
			$this->db->join('matchmst m', 'mf.MatchID=m.MstCode', 'INNER');
		...
		function getDataById($userId,$UserType){
			$this->db->select("mstruserid,partner,parentId,mstrid as usecode,mstrname,mstruserid as usename,active,usetype,mstrlock,FN_getUserChips(cm.mstrid) as P_L");
			$this->db->where('usetype > ',$UserType);
			$this->db->where('parentId',$userId);
			$this->db->from('createmaster cm');
			$this->db->order_by('cm.mstrname', 'ASC');
			$query = $this->db->get();
			//echo $this->db->queries[0];die();		
			return $query->result_array();
		}
		...
		function getFancylistHeader(){
			$this->db->select("mf.ID, mchmst.matchName,mf.HeadName,mf.active,mf.result,mf.TypeID,mchmst.SportID as SportID,mf.MatchID,marketId");
			$this->db->from('matchfancy mf');
		...
		function updateFancyHeaderSatatus(){
			...
			$dataArray = array('active' => $_POST['active'],'HelperID' => $_POST['HelperID'],'autoActive' => $_POST['autoActive']);
		...
		function closeUserList(){
			$this->db->select("mstrid,mstrname,mstruserid,usecrdt,lgnusrCloseAc, CASE WHEN usetype=0 THEN 'ADMIN' ELSE CASE WHEN usetype=1 THEN 'Master' ELSE CASE WHEN usetype=2 THEN 'Dealer' ELSE CASE WHEN usetype=3 THEN 'Client'  END  END  END END as UserType");
			$this->db->from('createmaster');
