...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class UsereventCntr extends CI_Controller {
		...
		function getMarketOfMatch($MatchId=null){
			...
			$data['marketfrmApi'] =  $this->getNextUkHorseRacingMarket($this->APP_KEY, $sessionToken, $matchID);
			$data['marketfrmDataBase'] = $this->Modeleventlst->GetMarketFrmDatabase();
			$this->output->set_content_type('application/json')->set_output(json_encode($data));
			$markeetName=json_encode($horseRacingEventTypeId);
		}

		function getBackLaysOfMarket($MarketId=null,$MatchId=null){		
			$sessionToken=$this->getTokenId();
			$data['MarketRunner'] = $this->getMarketBook($this->APP_KEY, $sessionToken, $MarketId);
			echo json_encode($data);
		}
		function chkmatch_result(){
			$data['MatchOddsVolVal']=$this->Modelcreatemaster->getMatchOddsLimit($MatchId1,$MarketId1);
		...
		function matchMarketLst(){
			...
			$data['MatchMarket'] = $this->Modeleventlst->matchMarketLst();
			//print_r($data['MatchMarket'][0]['FancyLen']);die();
			if($data['MatchMarket'][0]['FancyLen'] >0){
				$data['getMatchFancy']=$this->Modeleventlst->matchFancyList();
			}
			$this->output->set_content_type('application/json')->set_output(json_encode($data));	
