...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Createmastercontroller extends CI_Controller {
		...
		function submitClearChip(){//sourabh 161229
			...
			echo json_encode(array('error' => $condition[0]['resultV'] ,'message' => $condition[0]['retMess'],'txnId'=>$condition[0]['LID']));
		}
		function RollBackSettlement($txnId){//sourabh 161229
			$condition=$this->Modelcreatemaster->RollBackSettlement($txnId);
			echo json_encode(array('error' => $condition[0]['resultV'] ,'message' => $condition[0]['retMess'],'txnId'=>$condition[0]['LID']));
		}
		...
		function checkAutoFancyisActive(){
			   $MatchId=$_GET['MatchId'];

			$data['result']=$this->Modelcreatemaster->checkAutoFancyisActive($MatchId);
			$this->output->set_content_type('application/json')->set_output(json_encode($data));
		}

		function updatefancyAutoSetToUserPannel(){
			
                 $ID =$_GET['ID'];
                 $condition=$this->Modelcreatemaster->updatefancyAutoSetToUserPannel($ID);
			if ($condition==1) {
				echo json_encode(array('error' => 0 ,'message' => 'Fancy Addedd Succesfully...'));			
			}else{
				echo json_encode(array('error' => 1 ,'message' => 'Fancy Not Updated...'));
			}	

		}
		
		function SaveFancyApi(){
			$condition=$this->Modelcreatemaster->SaveFancyApi();
			if ($condition==1) {				
				echo json_encode(array('error' => 0 ,'message' => 'Fancy Inserted Successfully...'));
			}else if($condition==2){
				echo json_encode(array('error' => 1 ,'message' => 'Fancy Alredy Exits'));
