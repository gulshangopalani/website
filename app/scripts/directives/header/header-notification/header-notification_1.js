        ...
        controller: function ($scope, $http, loginService, $mdDialog, sessionService, $rootScope, get_userser, Dialog,$state) {
            ...
            $scope.displayUserInfo=function(){
                $mdDialog.show({
                    ...
                    templateUrl: 'app/scripts/directives/popupform/All_libility.html?var='+Math.random(),
                    locals: { prntScope: $scope },
                    clickOutsideToClose: true,
                    fullscreen: false,
                });
            }
            function displayUserDeatailsCntr($scope, $mdDialog, prntScope, sessionService) {
                    $scope.UserName=sessionService.get('slctUseName');
