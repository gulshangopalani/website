app.controller('PnlcntrBybId', function ($scope, $http, $filter, sessionService, $mdDialog, $rootScope, $location,$stateParams,$state) {
    ...
    $scope.doTheBack = function (GTuserType, GTUseruserId, GTUserName) {
        $http.get('Lstsavemstrcontroller/getParentData/' + GTUseruserId).success(function (data, status, headers, config) {
            ...
            $scope.GetPlus_Minus_Ac(data.parentData[0].mstrid,$stateParams.MarketId.matchId,$stateParams.MarketId.MarketId,$stateParams.MarketId.fancyId, data.parentData[0].mstruserid,data.parentData[0].usetype);
    ...
    $scope.MarketId=$stateParams.MarketId;
     var columnDefs = [
        ...
        { headerName: "Match/Unmtach", width: 80, field: "betType", cellClass: function (params) { return (params.data.Type == 'Lay' ? 'lay-head  col-status' : 'back-head  col-status'); } },
    ...
    $scope.GetPnL=function(){
        ...
        $http.get('Betentrycntr/BetHistoryPL/' + $stateParams.MarketId.UserId+'/'+$stateParams.MarketId.matchId+'/'+$stateParams.MarketId.MarketId+'/'+$stateParams.MarketId.fancyId).success(function (data, status, headers, config) {
            $scope.BetHistory = data.getBetPl;
            setRowData(data.getBetPl);
    ...
    $scope.GetPnL();
    $scope.GetPlus_Minus_Ac($stateParams.MarketId.UserId,$stateParams.MarketId.matchId,$stateParams.MarketId.MarketId,$stateParams.MarketId.fancyId,$stateParams.MarketId.mstruserid,$stateParams.MarketId.usetype);
