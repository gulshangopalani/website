app.controller('Editfancycntr', function ($scope, $rootScope, $http, $stateParams, Dialog, $mdDialog, $location,get_userser) {
    ...
    $scope.NormalFancy = function (Fancy) {
        if (($scope.userForm.$valid == true)) {
            if (($scope.NoLayRange == undefined || $scope.NoLayRange < Fancy.example) && ($scope.YesLayRange == undefined || $scope.YesLayRange > Fancy.example_1)) {
                var formData = { status: Fancy.fStatus, FancyId: parseInt(Fancy.FancyId), NoVal: Fancy.example, YesVal: Fancy.example_1, MaxStake: Fancy.MaxStake, NoValume: Fancy.NoValume, YesValume: Fancy.YesValume, pointDiff: Fancy.pointDiff,rateDiff:$scope.rateDiff,matchId:$stateParams.MatchID };
